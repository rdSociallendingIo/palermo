        ...
        $scope.init=function()
        {
            ...
            if($scope.txId!==undefined) { // add a test to check if it match tx paterns to avoid useless API call, clients are not obliged to come from the search form...
                ...
                getTransactionInfos()
                    ...
                    console.info("receiptRes : " + JSON.stringify(result.logs));
            ...
            function getTransactionInfos(){
                ...
                web3.eth.getTransaction($scope.txId,function(error, result) {
                    if(!error){
                        web3.eth.getTransactionReceipt($scope.txId, function (error, receiptRes) {
                           if (!error) {
                               if (receiptRes.logs && receiptRes.logs.length > 0) {
                                   result.contractAddress = receiptRes.address;
                                   var logArray = receiptRes.logs;
                                   var logs = [];
                                   var size = logArray.length;
                                   for (var i=0; i<size; i++) {
                                       console.info("i >>> ", i);
                                        logs.push(logArray[i].topics);
                                   }
                                   result.logs = logs;
                               }
